...
\begin{tikzpicture}
    ...
    \draw[thick] (0,-0.3) -- (0,+0.3) node[above] {$\overline{x}$};
    \draw[thick] (-1.5,+0.3) -- (-1.5,-0.3) node[below] {$\overline{y}_1$};
    \draw[thick] (+2.8,+0.3) -- (+2.8,-0.3) node[below] {$\overline{y}_2$};

    \node[scale=2] at (+4,0) {$]$};
    \node[scale=2] at (-4,0) {$($};

    \foreach \x in {0,...,15}{
            \filldraw[fill=red,draw=red] ({3.8*pow(0.65,\x)},0) circle (2pt);
        }
    \foreach \x in {0,...,15}{
            \filldraw[fill=red,draw=red] ({-3.8*pow(0.65,\x)},0) circle (2pt);
        }

    \draw[{Latex[length=2mm]}-{Latex[length=2mm]}] (-4,-1.3) -- (+4,-1.3) node[midway, below] {$\mathrm{dom} \, f :$ convex set};
\end{tikzpicture}

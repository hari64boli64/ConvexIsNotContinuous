...
\begin{tikzpicture}
  ...
  \draw[domain=-5:4, smooth, variable=\x, thick]
  plot ({\x}, {\f(\x)});

  \draw[very thick,dotted] (-5,{\f(-5)}) -- (-5, {\f(-5)+1});

  \draw[line width=1pt,fill=black] (-5, {\f(-5)+1}) circle (3pt);
  \draw[line width=1pt,fill=white] (-5, {\f(-5)}) circle (3pt);
  \draw[line width=1pt,fill=black] (4, {\f(4)}) circle (3pt);
\end{tikzpicture}

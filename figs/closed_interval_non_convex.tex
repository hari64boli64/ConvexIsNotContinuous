...
\begin{tikzpicture}
  \def\f(#1){0.2*#1*#1 + 0.6*#1 + 1.5}

  \draw[->] (-6,0) -- (5,0) node[below=0.5em] {\Large$x$};
  \draw[domain=-5:0, smooth, variable=\x, thick]
  plot ({\x}, {\f(\x)});
  \draw[domain=0:4, smooth, variable=\x, thick]
  plot ({\x}, {\f(\x)+1});

  \draw[very thick,dotted] (0,{\f(0)}) -- (0,{\f(0)+1});

  \draw[line width=1pt,fill=black] (-5, {\f(-5)}) circle (3pt);
  \draw[line width=1pt,fill=white] (0, {\f(0)}) circle (3pt);
  \draw[line width=1pt,fill=black] (0, {\f(0)+1}) circle (3pt);
  \draw[line width=1pt,fill=black] (4, {\f(4)+1}) circle (3pt);
\end{tikzpicture}
